...
split_time = int(float(time.mktime(time.strptime("2011-01-01T00:00:00Z", "%Y-%m-%dT%H:%M:%SZ"))))
...
item2class_dict = {}
with open('item2class_dict.pkl', 'rb') as f:
    item2class_dict = pickle.load(f)
...
def dataset_split(df):
    df_pretrain = df[df['timestamp']<split_time]
    df_downstream = df[df['timestamp']>=split_time]
    return df_pretrain, df_downstream


def df_normalize():
    df = pd.read_csv('./gowalla_cate_processed.csv', header=0)
    df['idx'] = [i for i in range(df.shape[0])]
    df.rename(columns={'userid':'u'},inplace=True)
    df.rename(columns={'placeid':'i'},inplace=True)
...
def time_transfer_data(df, split, max_node_id, max_edge_id):
    for cla in classes:
        ...
        OUT_DF = './time_trans/ml_gowalla_{}_{}.csv'.format(cla, split)
        OUT_FEAT = './time_trans/ml_gowalla_{}_{}.npy'.format(cla, split)
        OUT_NODE_FEAT = './time_trans/ml_gowalla_{}_{}_node.npy'.format(cla, split)
...
def field_transfer_data(df, max_node_id, max_edge_id):
    for cla in classes:
        ...
        OUT_DF = './field_trans/ml_gowalla_{}.csv'.format(cla)
        OUT_FEAT = './field_trans/ml_gowalla_{}.npy'.format(cla)
        OUT_NODE_FEAT = './field_trans/ml_gowalla_{}_{}_node.npy'.format(cla)
        user_map = data_stat(result_df, 'u')
        item_map = data_stat(result_df, 'i')
        user_count, item_count, example_count = len(user_map), len(item_map), result_df.shape[0]
        node_feat = np.random.uniform(size=(max_node_id + 1, 64))
